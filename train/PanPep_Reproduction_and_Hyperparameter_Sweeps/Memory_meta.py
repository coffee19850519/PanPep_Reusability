...
class Memory_Meta(nn.Module):
    ...
    def __init__(self, args, config):
        ...
        self.prev_loss = []
        self.prev_data = []
        self.training_scores = []  # Store all scores during training
        if torch.cuda.is_available():
            self.models = torch.Tensor().cuda()
        else:
            self.models = torch.Tensor().cpu()
    ...
    def forward(self, peptide, x_spt, y_spt, x_qry, y_qry, epoch=None,
                peptide_seqs=None, spt_tcr_seqs=None, qry_tcr_seqs=None):
        """
        ...
        Parameters:
            ...
            param y_qry: the labels of query set
            param epoch: current training epoch (optional, for score tracking)
            param peptide_seqs: list of peptide sequences (optional, for score tracking)
            param spt_tcr_seqs: list of support TCR sequences (optional, for score tracking)
            param qry_tcr_seqs: list of query TCR sequences (optional, for score tracking)
        ...
        for i in range(task_num):
            ...
            # Collect support set scores (initial)
            if epoch is not None and peptide_seqs is not None and spt_tcr_seqs is not None:
                with torch.no_grad():
                    support_scores = F.softmax(logits, dim=1)[:, 1].cpu().numpy()
                    for sample_idx in range(len(support_scores)):
                        self.training_scores.append({
                            'epoch': epoch,
                            'task_idx': i,
                            'sample_type': 'support_initial',
                            'update_step': 0,
                            'sample_idx': sample_idx,
                            'peptide': peptide_seqs[i],
                            'tcr': spt_tcr_seqs[sample_idx],
                            'label': y_spt[i][sample_idx].item(),
                            'score': support_scores[sample_idx]
                        })
            ...
            # Collect query set scores (initial, before any update)
            if epoch is not None and peptide_seqs is not None and qry_tcr_seqs is not None:
                with torch.no_grad():
                    query_scores = F.softmax(logits_q, dim=1)[:, 1].cpu().numpy()
                    for sample_idx in range(len(query_scores)):
                        self.training_scores.append({
                            'epoch': epoch,
                            'task_idx': i,
                            'sample_type': 'query_initial',
                            'update_step': 0,
                            'sample_idx': sample_idx,
                            'peptide': peptide_seqs[i],
                            'tcr': qry_tcr_seqs[sample_idx],
                            'label': y_qry[i][sample_idx].item(),
                            'score': query_scores[sample_idx]
                        })

            # save the loss of meta learner
            loss_start = loss_q
            # print("start support_loss:", float(loss.cpu().detach().numpy()), "query_loss", float(loss_q.cpu().detach().numpy()))
            ...
            if self.update_step == 1:
                # [setsz, nway]
                # combined_weights = list(map(lambda p: (1-alpha) * p[0] + alpha * p[1], zip(fast_weights, weights_memory)))
                logits_q = self.net(x_qry[i], fast_weights, bn_training=True)
                loss_q = F.cross_entropy(logits_q, y_qry[i])
                losses_q[1] += loss_q

                # Collect query set scores (after first update)
                if epoch is not None and peptide_seqs is not None and qry_tcr_seqs is not None:
                    with torch.no_grad():
                        query_scores = F.softmax(logits_q, dim=1)[:, 1].cpu().numpy()
                        for sample_idx in range(len(query_scores)):
                            self.training_scores.append({
                                'epoch': epoch,
                                'task_idx': i,
                                'sample_type': 'query_after_update',
                                'update_step': 1,
                                'sample_idx': sample_idx,
                                'peptide': peptide_seqs[i],
                                'tcr': qry_tcr_seqs[sample_idx],
                ...
                with torch.no_grad():
                    ...
                    # Collect query set scores (after first update, update_step > 1)
                    if epoch is not None and peptide_seqs is not None and qry_tcr_seqs is not None:
                        query_scores = F.softmax(logits_q, dim=1)[:, 1].cpu().numpy()
                        for sample_idx in range(len(query_scores)):
                            self.training_scores.append({
                    ...
                    correct = torch.eq(pred_q, y_qry[i]).sum().item()
                    corrects[1] = corrects[1] + correct
                    # print("support_loss:", float(loss.cpu().detach().numpy()), "query_loss", float(loss_q.cpu().detach().numpy()))
                ...
                for k in range(1, self.update_step):
                    ...
                    # Collect support set scores (after k-th update)
                    if epoch is not None and peptide_seqs is not None and spt_tcr_seqs is not None:
                        with torch.no_grad():
                            support_scores = F.softmax(logits, dim=1)[:, 1].cpu().numpy()
                            for sample_idx in range(len(support_scores)):
                                self.training_scores.append({
                                    'epoch': epoch,
                                    'task_idx': i,
                                    'sample_type': 'support_after_update',
                                    'update_step': k,
                                    'sample_idx': sample_idx,
                                    'peptide': peptide_seqs[i],
                                    'tcr': spt_tcr_seqs[sample_idx],
                    ...
                    loss_q = F.cross_entropy(logits_q, y_qry[i])
                    losses_q[k + 1] += loss_q
                    # print("update_step",k , "support_loss:", float(loss.cpu().detach().numpy()), "query_loss", float(loss_q.cpu().detach().numpy()))

                    # Collect query set scores (after k-th update)
                    if epoch is not None and peptide_seqs is not None and qry_tcr_seqs is not None:
                        with torch.no_grad():
                            query_scores = F.softmax(logits_q, dim=1)[:, 1].cpu().numpy()
                            for sample_idx in range(len(query_scores)):
            ...
            # print("support_loss:", float(loss.cpu().detach().numpy()), "query_loss",
            #       float(loss_q.cpu().detach().numpy()))
    ...
    def save_training_scores(self, output_path, append=False):
        """
        Save all collected training scores to a parquet file.
        ...
        if len(self.training_scores) == 0:
            print("No training scores to save.")
            return

        # Convert list of dicts to DataFrame
        df = pd.DataFrame(self.training_scores)

        # Save to parquet
        if append and os.path.exists(output_path):
            fastparquet.write(output_path, df, append=True)
            print(f"Training scores appended to {output_path} ({len(df)} records)")
        else:
            fastparquet.write(output_path, df)
            print(f"Training scores saved to {output_path} ({len(df)} records)")