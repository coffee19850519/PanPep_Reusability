...
def parse_args():
    ...
    parser.add_argument('--upper_limit', type=int, default=400000, help='Upper limit for window size')
    parser.add_argument('--k_shot_rate', type=int, default=0.8, help='K-shot rate for positive TCRs')
...
def process_peptide_with_lock(pep, test_data, test_data_tcr_negative, model, aa_dict, args, config, device, result_dir, file_lock, peptide_encoding_dict, tcr_encoding_dict):
    ...
    kshot = int(config.k_shot*len(positive_tcr))
    ...
    window_count = math.ceil((len(all_ranking_data[pep][1]) - 2*kshot) / config.upper_limit)
    ...
    if config.kshot_dir:
        ...
        with file_lock:
            ...
            k_shot_data = save_kshot_data(all_ranking_data[pep], kshot , pep, result_dir)

    all_query_data = get_query_data(all_ranking_data[pep], k_shot_data, kshot)
