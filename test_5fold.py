...
if __name__ == '__main__':
    ...
    for r_idx in range(1, (Train_Round + 1)):
        ...
        for kfold_dir in os.listdir(round_dir):
            ...
            test_5fold_few_shot(model, F_data, output_file=os.path.join(test_output_dir, 'Few-shot_Result_Round_' + str(r_idx) + '_kfold_' + kfold_idx + '_test.csv'), aa_dict=aa_dict, device=device)
            test_5fold_zero_shot(model, F_data, output_file=os.path.join(test_output_dir, 'Zero-shot_Result_Round_' + str(r_idx) + '_kfold_' + kfold_idx + '_test.csv'), aa_dict=aa_dict, device=device)
